...
function scaleBg(cord, itemCount) {
  const offSet = ((cord / 100) * (itemCount * Math.PI)) / 100;
  return `scale(${1 + cord / 30 + offSet})`;
}
...
const Remote = props => {
  ...
  const text = name ? name : group.group.name;
  const sceneArray = Object.entries(group.scenes);
  ...
  const [{ xCordsChildren, zIndex }, setTransformChildSpring] = useSpring(
    () => ({
      xCordsChildren: 0,
      zIndex: 50,
    })
  ...
  const [{ x, y }, setMoveSpring] = useSpring(() => ({ x: 0, y: 0 }));
  ...
  useGetLocalStorageCoordinates(remoteId, lastLocalXY, setMoveSpring, config);
  ...
  const [containerSize] = useGetContainerSize(componentRef);
  ...
  return (
    ...
    >
      <div className={styles.iconContainer}>
          ...
          {sceneArray.map((scene, i) => {
            ...
            return (
              <SceneIcon
                ...
                number={i}
                numberOfChilds={sceneArray.length}
                text={scene[1].name}
                movable={movable}
                parentId={remoteId}
                parentName={text}
        ...
        <span
        // style={{ opacity: xCordsChildren.interpolate(c => -(c / 100) + 1) }}
        >
          {text.toUpperCase()}
        </span>
      </div>
      ...
      <animated.div
        ...
        style={{
          // transform: xCordsChildren.interpolate(c => `scale(${1 + c / 40})`),
          transform: xCordsChildren.interpolate(c =>
            scaleBg(c, sceneArray.length)
          ),
        }}
      />
    </animated.div>
